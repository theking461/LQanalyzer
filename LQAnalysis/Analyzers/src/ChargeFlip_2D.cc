// $Id: ChargeFlip_2D.cc 1 2013-11-26 10:23:10Z jalmond $
/***************************************************************************
 * @Project: LQChargeFlip_2D Frame - ROOT-based analysis framework for Korea SNU
 * @Package: LQCycles
 *
...
ClassImp (ChargeFlip_2D);
  ...
  *
  */
ChargeFlip_2D::ChargeFlip_2D() :  AnalyzerCore(), out_muons(0)  {
  
  
  // To have the correct name in the log:                                                                                                                            
  SetLogName("ChargeFlip_2D");
  
  Message("In ChargeFlip_2D constructor", INFO);
  //
...
void ChargeFlip_2D::InitialiseAnalysis() throw( LQError ) {
...
void ChargeFlip_2D::ExecuteEvents()throw( LQError ){
  ...
  */
  ...
  if(electrons.at(0).Charge() * truthColl.at(first_e_match_i).PdgId() > 0){
    ...
    FillHist("gen_pt_VS_reco_pt_CF", electrons.at(0).Pt(), truthColl.at(first_e_match_i).Pt(), 1., 0., 500., 100., 0., 500., 100);    
    e1_cf_suoh = false;
  }
  //2nd e
  ...
  if(electrons.at(1).Charge() * truthColl.at(second_e_match_i).PdgId() > 0){
    ...
    FillHist("gen_pt_VS_reco_pt_CF", electrons.at(1).Pt(), truthColl.at(second_e_match_i).Pt(), 1., 0., 500., 100., 0., 500., 100);
    e2_cf_suoh = false;
  }
...
void ChargeFlip_2D::EndCycle()throw( LQError ){
...
void ChargeFlip_2D::BeginCycle() throw( LQError ){
...
ChargeFlip_2D::~ChargeFlip_2D() {
  
  Message("In ChargeFlip_2D Destructor" , INFO);
  //if(!k_isdata)delete reweightPU;
  
}


void ChargeFlip_2D::FillCutFlow(TString cut, float weight){
...
void ChargeFlip_2D::BeginEvent( )throw( LQError ){
...
void ChargeFlip_2D::MakeHistograms(){
  ...
  /**
   *  Remove//Overide this ChargeFlip_2DCore::MakeHistograms() to make new hists for your analysis
   **/
  
}


void ChargeFlip_2D::ClearOutputVectors() throw(LQError) {
