// $Id: Bgd_Fake_control_MM.cc 1 2013-11-26 10:23:10Z jalmond $
/***************************************************************************
 * @Project: LQBgd_Fake_control_MM Frame - ROOT-based analysis framework for Korea SNU
 * @Package: LQCycles
 *
...
ClassImp (Bgd_Fake_control_MM);
  ...
  *
  */
Bgd_Fake_control_MM::Bgd_Fake_control_MM() :  AnalyzerCore(), out_muons(0)  {
  
  
  // To have the correct name in the log:                                                                                                                            
  SetLogName("Bgd_Fake_control_MM");
  
  Message("In Bgd_Fake_control_MM constructor", INFO);
  //
...
void Bgd_Fake_control_MM::InitialiseAnalysis() throw( LQError ) {
...
void Bgd_Fake_control_MM::ExecuteEvents()throw( LQError ){
  ...
  */
  ...
  //fake control region, DY + 1 fake lepton
  if(muons.size() == 3){
    snu::KParticle Z1 = muons.at(0) + muons.at(1) + muons.at(2);
    float minimum = 99999.;
    int min_i = 0.;
    
    for(int i = 0; i < muons.size(); i ++){
      Z1 = Z1 - muons.at(i);
      float current_dM = fabs(Z1.M() - 90.0);
      if(current_dM < minimum){
	minimum = current_dM;
	min_i = i;
      }
    }
  ...
  if(jets.size() > 3 && MET > 75){
    ...
    FillCLHist(hnpairmm,"CR_1_tempPU", eventbase->GetEvent(), muons, electrons,jets, current_weight * temp_pileup_reweight, n_bjet);
  }
  if(jets.size() > 2 && muons.size() > 0){
    if(muons.at(0).Pt() < 50){
      ...
      FillCLHist(hnpairmm,"CR_2_tempPU", eventbase->GetEvent(), muons, electrons,jets, current_weight * temp_pileup_reweight, n_bjet);
    }
  }
  if(muons.size() > 0){
    if(muons.at(0).Pt() < 50){
      FillCLHist(hnpairmm,"CR_3_PU", eventbase->GetEvent(), muons, electrons,jets, current_weight * pileup_reweight, n_bjet);
      FillCLHist(hnpairmm,"CR_3_tempPU", eventbase->GetEvent(), muons, electrons,jets, current_weight * temp_pileup_reweight, n_bjet);
    }
  }
  if(muons.size() > 1){
...
void Bgd_Fake_control_MM::EndCycle()throw( LQError ){
...
void Bgd_Fake_control_MM::BeginCycle() throw( LQError ){
...
Bgd_Fake_control_MM::~Bgd_Fake_control_MM() {
  
  Message("In Bgd_Fake_control_MM Destructor" , INFO);
  if(!k_isdata)delete reweightPU;
  
}


void Bgd_Fake_control_MM::FillCutFlow(TString cut, float weight){
...
void Bgd_Fake_control_MM::BeginEvent( )throw( LQError ){
...
void Bgd_Fake_control_MM::MakeHistograms(){
  ...
  /**
   *  Remove//Overide this Bgd_Fake_control_MMCore::MakeHistograms() to make new hists for your analysis
   **/
  
}


void Bgd_Fake_control_MM::ClearOutputVectors() throw(LQError) {
