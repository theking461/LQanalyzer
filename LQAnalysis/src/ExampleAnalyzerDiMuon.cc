...
void ExampleAnalyzerDiMuon::ExecuteEvents()throw( LQError ){
  ...
  */
  ...
  if(k_running_nonprompt) muid= BaseSelection::MUON_POG_LOOSE;
  ...
  if(!isData){
    ...
    current_weight = weight * id_iso_sf * trigger_ps * trigger_sf * trig_pass;
  }
  /*
  ...
  */
  ...
  //OS
  if(muons.size() == 2 && muons_veto.size() == 2 && electrons_veto.size() == 0 && muons.at(0).Charge() != muons.at(1).Charge() && trig_pass && !k_running_nonprompt){          
    ...
    if(muons.at(1).Pt() > 20. && Z.M() > 10 && trig_pass){
	...
	FillHist("eta_2nd_lepton_1jet_OS_tempPU", muons.at(1).Eta() , current_weight * temp_pileup_reweight, -5. , 5., 1000);
      ...
      //if(jets.size() > 1 && jets.at( jets.size() - 1 ).Pt() > 20){
      if(jets.size() > 1 && jets.at( Njet - 1 ).Pt() > 20){   	 
	float min_jet_pt = 9999.;
	int first_jet = 0;
	int second_jet = 1;
	
	
	for(int i = 0; i < Njet; i++){
	  //if(jets.at(i).Pt() > jets.at(first_jet).Pt()) first_jet = i;
	  //if(jets.at(i).Pt() < jets.at(first_jet).Pt() && jets.at(i).Pt() > jets.at(second_jet).Pt()) second_jet = i;
	  if(jets.at(i).Pt() < min_jet_pt) min_jet_pt = jets.at(i).Pt();
	}
	
	if(jets.at(0).Pt() < 20 && min_jet_pt > 20){
	  cout << "Pt 1st jet : " << jets.at(0).Pt() << endl;
	  cout << "Pt 2nd jet : " << jets.at(1).Pt() << endl;
	  cout << "Pt Njet -1 jet : " << jets.at( Njet - 1 ).Pt() << endl;
	}
	
	if(jets.at(0).Pt() > jets.at(1).Pt()) FillHist("Jet_order", 0. , 1., 0. , 2., 2);
	if(jets.at(0).Pt() < jets.at(1).Pt()) FillHist("Jet_order", 1. , 1., 0. , 2., 2);
	...
	if(min_jet_pt > 20){
	  ...
	  FillHist("mass_jj_2jet_OS_PU", W.M(), current_weight * pileup_reweight, 0., 1000., 1000);
	  FillHist("mass_lljj_2jet_OS_PU", (W + Z).M(), current_weight * pileup_reweight, 0., 1000., 1000);
	  FillHist("MET_2jet_OS_PU", MET, current_weight * pileup_reweight, 0., 1000., 1000);

	  FillHist("mass_jj_2jet_OS_tempPU", W.M(), current_weight * temp_pileup_reweight, 0., 1000., 1000);
          FillHist("mass_lljj_2jet_OS_tempPU", (W + Z).M(), current_weight * temp_pileup_reweight, 0., 1000., 1000);
  ...
  if(muons.size() == 2 && muons_veto.size() == 2 && electrons_veto.size() == 0 && muons.at(0).Charge() == muons.at(1).Charge() && trig_pass) {
    ...
    if(muons.at(1).Pt() > 20. && Z.M() > 10 && prompt_match && trig_pass){
	...
	if(min_jet_pt > 20){
	  ...
	  snu::KParticle W = jets.at(0) + jets.at(1);
	  FillHist("mass_jj_2jet_SS_PU", W.M(), current_weight * pileup_reweight, 0., 1000., 1000);
	  FillHist("mass_lljj_2jet_SS_PU", (W + Z).M(), current_weight * pileup_reweight, 0., 1000., 1000);
	  FillHist("MET_2jet_SS_PU", MET, current_weight * pileup_reweight, 0., 1000., 1000);
	  
	  FillHist("mass_jj_2jet_SS_tempPU", W.M(), current_weight * temp_pileup_reweight, 0., 1000., 1000);
          FillHist("mass_lljj_2jet_SS_tempPU", (W + Z).M(), current_weight * temp_pileup_reweight, 0., 1000., 1000);
