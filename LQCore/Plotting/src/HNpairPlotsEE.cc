...
HNpairPlotsEE::HNpairPlotsEE(TString name): StdPlots(name){
...
void HNpairPlotsEE::Fill(snu::KEvent ev, std::vector<snu::KMuon>& muons, std::vector<snu::KElectron>& electrons, std::vector<snu::KJet>& jets, Double_t weight, int nbjet) {
  ...
  if( (electrons[0] + electrons[1]).M() < 10 ) return;

  if( electrons[0].Pt() < 30 || electrons[1].Pt() < 20 ) return;
  ...
  if(jets.size() > 3){
    ...
    for(int j1 = 0; j1 < 4; j1 ++){
      if(electrons[0].DeltaR(jets[j1]) < min_del_R_mu1j) min_del_R_mu1j = electrons[0].DeltaR(jets[j1]);
      if(electrons[1].DeltaR(jets[j1]) < min_del_R_mu2j) min_del_R_mu2j = electrons[1].DeltaR(jets[j1]);
      
      for(int j2 = 0; j2 < 4; j2 ++){
	snu::KParticle all_jet = jets[0] + jets[1] + jets[2] + jets[3];
	snu::KParticle current_HN1 = electrons[0] + jets[j1] + jets[j2];
	all_jet = all_jet - jets[j1] - jets[j2];
	snu::KParticle current_HN2 = electrons[1] + all_jet;
	...
	current_HN1 = electrons[0] + all_jet;
	current_HN2 = electrons[1] + jets[j1] + jets[j2];
	if(fabs(current_HN1.M() - current_HN2.M()) < min_del_HNHN && j1 != j2){
	  min_del_HNHN = fabs(current_HN1.M() - current_HN2.M());
	  HN_1 = current_HN1;
	  HN_2 = current_HN2;
  ...
  if(electrons[0].Charge() != electrons[1].Charge()){
    //CR
    Fill("h_llmass_OS", (electrons[0]+electrons[1]).M(), weight);    
    Fill("h_leadingLeptonPt_OS", electrons[0].Pt(), weight);
    Fill("h_secondLeptonPt_OS", electrons[1].Pt(), weight);
    Fill("h_leadingLeptonEta_OS", electrons[0].Eta(), weight);
    Fill("h_secondLeptonEta_OS", electrons[1].Eta(), weight);
    ...
    //SR
    if(jets.size() > 3){
      snu::KParticle Zp = electrons[0] + electrons[1] + jets[0] + jets[1] + jets[2] + jets[3];
  ...
  if(electrons[0].Charge() == electrons[1].Charge()){
    //CR
    Fill("h_llmass_SS", (electrons[0]+electrons[1]).M(), weight);
    Fill("h_leadingLeptonPt_SS", electrons[0].Pt(), weight);
    Fill("h_secondLeptonPt_SS", electrons[1].Pt(), weight);
    Fill("h_leadingLeptonEta_SS", electrons[0].Eta(), weight);
    Fill("h_secondLeptonEta_SS", electrons[1].Eta(), weight);
...
HNpairPlotsEE::HNpairPlotsEE(): StdPlots(){
}
 ...
 * Copy constructor.
 */
HNpairPlotsEE::HNpairPlotsEE(const HNpairPlotsEE& sp): StdPlots(sp)
{
...
HNpairPlotsEE& HNpairPlotsEE::operator= (const HNpairPlotsEE& sp)
{
  if (this != &sp) {
...
void HNpairPlotsEE::Fill(TString name, double value, double w){
...
void HNpairPlotsEE::Fill(TString name, double value1, double value2, double w){
...
void HNpairPlotsEE::Fill(TString name, double value1, double value2, double value3, double w){
